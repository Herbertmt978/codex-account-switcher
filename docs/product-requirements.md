...
At startup, if Codex's current login differs from the recorded active profile and matches exactly one saved profile, the switcher selects that profile automatically without copying or replacing any credential. If no profile or more than one profile matches, it leaves the registry unchanged and asks the user to register the current account. Before closing Desktop, switching verifies the current and target saved identities and stops without interrupting Desktop if either cannot be confirmed. A failed handoff after Desktop closes restores the previous credential where needed and reopens Desktop before reporting the error; if restoration itself fails, it leaves Desktop closed and reports that failure.
